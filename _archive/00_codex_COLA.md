...
```text
E:\COKECOLA\CODEX\00_SKILL\15-knowledge-vault-workflow.md
```
...
- `Clipping`：放外部來源資料，例如 YouTube 字幕、網頁文章、GitHub README、PDF OCR。
- `創作庫`：放自己的 prompt、課程草稿、腳本、專案想法。
- `知識庫`：放 Agent 整理後的長期知識、索引、主題筆記與整理紀錄。
...
注意：該 repo 適合作為藍圖參考，但原始腳本綁定固定頻道與固定路徑，不建議直接照抄執行。COLA 版本請以 `15-knowledge-vault-workflow.md` 為準。
...
[ ] ui-ux-pro-max 已安裝
[ ] create-yourself 已安裝
[ ] 04_KNOWLEDGE_VAULT 已建立
[ ] 00_SKILL\15-knowledge-vault-workflow.md 已建立
```
