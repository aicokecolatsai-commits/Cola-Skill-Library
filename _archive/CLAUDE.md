...
## 📋 00_SKILL 盤點結構（COLA 版）

這個目錄除了是 Math Ruffian 的懶人包倉庫，也被 COLA 用來收集網路上的各類 SKILL。整理文件集中在 `_docs/`：

```
00_SKILL/
├── _docs/
│   ├── 00-SKILL總盤點.md      ← 🔥 全部 SKILL 用途 + 安裝方式
│   └── 01-環境建置清單.md      ← 跨平台驗證清單
├── 00-env-setup/ ~ 13-chezmoi/ ← Math Ruffian 懶人包技能
├── 14-codex-skills/            ← 社群/系統 SKILL 備份
├── 00-環境建置.md ~ 08-*.md    ← 原始說明文件（勿刪）
├── COLA.md / COLAOPENCODE.md   ← 個人環境盤點
└── README.md / SKILL.md        ← 原始懶人包入口
```

**跨平台通用：** OpenCode / Codex / Antigravity 三平台皆可用相同的 SKILL.md 格式。
安裝方式詳見 `_docs/00-SKILL總盤點.md` 第 3 節。
